...
def load_excel_workbook(file, strict: bool = True) -> tuple:
    ...
    `strict=True` (default, used for the single "Upload Daily Excel"
    path) requires all 5 sheets to be present -- this is meant to be one
    complete daily snapshot.
    `strict=False` (used by consolidate.py for per-RM files) tolerates
    missing sheets -- a per-RM export commonly only has Partner Master /
    Client Master / Transaction Fact, with Partner Target / Partner
    Review maintained centrally in a separate file in the same folder.
    Missing sheets become empty (but correctly-columned) DataFrames.
    """
    messages = []
    try:

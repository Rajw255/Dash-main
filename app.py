...
with col_inputs:
    ...
    sip_per_client = st.number_input("SIP / Client / Month (₹)", min_value=0, value=0, step=500, key="ic_sip_amt")
    sip_stepup = st.number_input("SIP Step-up % p.a.", min_value=0.0, value=1, step=0.5, key="ic_sip_stepup")
    lumpsum_amt = st.number_input("Annual Lumpsum / Client (₹)", min_value=0, value=0, step=1000, key="ic_lumpsum")
